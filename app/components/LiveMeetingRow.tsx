...
export const LiveMeetingRow: FC<LiveMeetingRowProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Meeting Name */}
      <Text style={themed($meetingName)} numberOfLines={1} accessible={false}>
        {meeting.name}
      </Text>

      {/* Indicator icons */}
      {meeting.external && <Text style={$externalZoomIcon} accessible={false}>Z</Text>}
      {hasReminder && (
        <Ionicons name="notifications" size={14} color={REMINDER_COLOR} style={$bellIcon} accessible={false} />
      )}
      {isFavorite && <Ionicons name="heart" size={16} color="#ef4444" style={$heartIcon} accessible={false} />}
